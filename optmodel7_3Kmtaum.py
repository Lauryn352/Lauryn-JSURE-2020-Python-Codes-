...
def fopdt(y,t,x,Km1,Km2,Km3,taum):
  ...
  #  try:
    if t <= 1:
                Km1 = x[0]
                dydt = Km1*y**(2/3)-taum*y 
    else: 
        if t <= 3:
                Km2 = x[1]
                dydt = Km2*y**(2/3)-taum*y 
        else:
                Km3 = x[2]
                dydt = Km3*y**(2/3)-taum*y
  #  except:
...
def sim_model(x):
  ...
  #              y1 = odeint(fopdt,ym[i],ts,args=(uf,Km,taum,thetam))
        y1 = odeint(fopdt,ym[i],ts,args=(x,Km1,Km2,Km3,taum))
        ym[i+1] = y1[-1]
    return ym
...
np.savetxt('datamd7.txt',data,delimiter=',')
plt.savefig('outputmodel73Kmtaum.png',dpi=300,bbox_inches='tight')
plt.show()